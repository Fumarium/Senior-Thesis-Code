# -*- coding: utf-8 -*-
"""Thesis_Code_PS.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/14de74JzQKlqk9NNUzGfkVVm6EVLY2GUC
"""
...
for i in range(0, num_files):
  csv_output_paths.append(f"/content/drive/MyDrive/Colab Notebooks/Senior Thesis/Data/CSV_cleaned/Updated_consolidated/1000NL_PS/ps_NLH_1000_{i+1}.csv")
  phhs_file_paths.append(f'/content/drive/MyDrive/Colab Notebooks/Senior Thesis/Data/data/handhq/PS-2009-07-01_2009-07-23_1000NLH_OBFU/10/ps NLH handhq_{i+1}-OBFUSCATED.phhs')
for i in range(0, num_files):
...
csv_folder = "/content/drive/MyDrive/Colab Notebooks/Senior Thesis/Data/CSV_cleaned/Updated_consolidated/1000NL_PS"
...
csv_folder = "/content/drive/MyDrive/Colab Notebooks/Senior Thesis/Data/CSV_cleaned/Updated_consolidated/1000NL_PS"
master_csv_path = "/content/drive/MyDrive/Colab Notebooks/Senior Thesis/Data/CSV_cleaned/Updated_consolidated/1000NL_PS/master_1000NL.csv"
...
master_file_path = "/content/drive/MyDrive/Colab Notebooks/Senior Thesis/Data/CSV_cleaned/Updated_consolidated/1000NL_PS/master_1000NL.csv"
...
master_exploded_filtered.to_csv("/content/drive/MyDrive/Colab Notebooks/Senior Thesis/Code/Redone/1000NL_master_exploded_filtered")

"""At this stage of the project, I have manufactured a dataset that contains hands played by individuals who played more than 50 hands on a given table, sorted in by table and time. We can now begin to think about sequences of hands. The ultimate goal is to create a dictionary-type object where the key is the player and the value is a the (sliding window adjusted) sequence of hands played by this player on a particular table in chronological order such that each value is in essence a time series. We want to end up with something like $$ \text{Alice}:\{S_1,S_2,...,S_k\}$$ where each $S_i$ is some data structure embedding information about its consituent sequence of hands.

I think the first order of business is to map out which hands belong to which player. This can be achieved by referencing the hand index from the master exploded, filtered dataset, so that for each player we can track which hands will constitute a corresponding sequence. Suppose we define a sequence length of 8 hands with a sliding window of 4 hands. Our resulting dataset will contain the following information: player, double array (sequences) where each element is the list of hand id's corresponding to the hands in the sequence.
"""
...
# print((master_exploded_filtered["starting_stacks"][0]))
...
with open('/content/drive/MyDrive/Colab Notebooks/Senior Thesis/Code/Redone/sequences_dictionary.pkl', 'wb') as f:
    pickle.dump(sequencesDictionary, f)

# with open('/content/drive/MyDrive/Colab Notebooks/Senior Thesis/Code/Redone/sequences_dictionary.pkl', 'rb') as f:
#     loaded_dict = pickle.load(f)

"""This is an example of a hand (in action terms):
[  'd dh p1 ????',  'd dh p2 ????',  'p2 cbr 30',     'p1 cc',  'd db 9d3d5h',
  'p1 cc',  'p2 cbr 48',     'p1 cc',  'd db 2c',       'p1 cc',  'p2 cc',
      'd db 7s',  'p1 cbr 110',    'p2 cc',  'p1 sm 9cQd',    'p2 sm ????']

Let us start with a discussion regarding the action format. We have the
following:

* d: indicates a dealer action or dealing of cards
* dh: "deal hole", this is the dealer distributing cards to a player
* db: "deal board", dealer deals community card
* cbr: "continuation bet/raise"-- the player has put chips in the pot
* cc: "check or call:-- this is generally a zero or matching bet action
* sm: "showdown"? Not sure about this but it's inconsequential
* px, where x is a player number, tells us which player is acting
* there is a numerical amount and either "????" or a card string which tells
us either what the player has at showdown or whether they mucked their cards.
"""
...
df['CompositeTiltIndicator'] = df.apply(compute_cti, axis=1)
df['NormalizedCTI'] = 1 / (1+np.exp(-df['CompositeTiltIndicator']))
df['Tilt_label'] = df['NormalizedCTI'].apply(lambda x: 1 if x >= 0.95 else 0)

csv_filename = "/content/drive/MyDrive/Colab Notebooks/Senior Thesis/Code/Redone/tilt_metrics_labeled_10p.csv"
...
df = pd.read_csv("/content/drive/MyDrive/Colab Notebooks/Senior Thesis/Code/Redone/PS_Output/tilt_metrics_labeled.csv")
df

# prompt: plot the distribution of average aggression using matplotlib

import matplotlib.pyplot as plt

# Assuming 'df' is your DataFrame and 'AverageAggression' is the column name
plt.figure(figsize=(10, 6))
plt.hist(df['AverageAggression'], bins=20, edgecolor='black', log=True)
plt.xlabel('Average Aggression')
plt.ylabel('Frequency')
# plt.title('Distribution of Average Aggression')
# plt.grid(True)
plt.show()
...
# Assuming 'df' is your DataFrame and 'AverageAggression' is the column name
plt.figure(figsize=(10, 6))
plt.hist(df['DynamicAggressionChange'], bins=20, edgecolor='black', log=True)
plt.xlabel('Dynamic Aggression Change')
plt.ylabel('Frequency')
# plt.title('Distribution of Dynamic Aggression Change')
# plt.grid(True)
plt.show()
...
# Assuming 'df' is your DataFrame and 'AverageAggression' is the column name
plt.figure(figsize=(10, 6))
plt.hist(df['PostLossAggressionRatio'], bins=20, edgecolor='black', log=True)
plt.xlabel('Post Loss Aggression Ratio')
plt.ylabel('Frequency')
# plt.title('Distribution of Dynamic Post Loss Aggression Ratio')
# plt.grid(True)
plt.show()
...
# Assuming 'df' is your DataFrame and 'AverageAggression' is the column name
plt.figure(figsize=(10, 6))
plt.hist(df['AggressionTrendSlope'], bins=20, edgecolor='black',log=True)
plt.xlabel('Aggression Trend Slope')
plt.ylabel('Frequency')
# plt.title('Distribution of Aggression Trend Slope')
# plt.grid(True)
plt.show()
...
# Assuming 'df' is your DataFrame and 'AverageAggression' is the column name
plt.figure(figsize=(10, 6))
plt.hist(df['MaxConsecutiveLosses'], bins=20, edgecolor='black',log=True)
plt.xlabel('Maximum Consecutive Losses')
plt.ylabel('Frequency')
# plt.title('Distribution of Maximum Consecutive Losses')
# plt.grid(True)
plt.show()
...
# Assuming 'df' is your DataFrame and 'AverageAggression' is the column name
plt.figure(figsize=(10, 6))
plt.hist(df['WeightedLossImpact'], bins=20, edgecolor='black', log=True)
plt.xlabel('Weighted Loss Impact')
plt.ylabel('Frequency')
# plt.title('Distribution of Weighted Loss Impact')
# plt.grid(True)
plt.show()
...
# Assuming 'df' is your DataFrame and 'AverageAggression' is the column name
plt.figure(figsize=(10, 6))
plt.hist(df['SignificantLossCount'], bins=20, edgecolor='black', log=True)
plt.xlabel('Significant Loss Count')
plt.ylabel('Frequency')
# plt.title('Distribution of Significant Loss Count')
# plt.grid(True)
plt.show()
...
# Assuming 'df' is your DataFrame and 'AverageAggression' is the column name
plt.figure(figsize=(10, 6))
plt.hist(df['AverageRisk'], bins=20, edgecolor='black', log=True)
plt.xlabel('Average Risk')
plt.ylabel('Frequency')
# plt.title('Distribution of Average Risk')
# plt.grid(True)
plt.show()
...
# Assuming 'df' is your DataFrame and 'AverageAggression' is the column name
plt.figure(figsize=(10, 6))
plt.hist(df['RiskVariance'], bins=20, edgecolor='black', log=True)
plt.xlabel('Risk Variance')
plt.ylabel('Frequency')
# plt.title('Distribution of Risk Variance')
# plt.grid(True)
plt.show()
...
# Assuming 'df' is your DataFrame and 'AverageAggression' is the column name
plt.figure(figsize=(10, 6))
plt.hist(df['MaxRisk'], bins=20, edgecolor='black', log=True,range=(0,1))
plt.xlabel('Maximum Risk')
plt.ylabel('Frequency')
# plt.title('Distribution of Maximum Risk')
# plt.grid(True)
plt.show()
...
# Assuming 'df' is your DataFrame and 'AverageAggression' is the column name
plt.figure(figsize=(10, 6))
plt.hist(df['NormalizedCTI'], bins=20, edgecolor='black')
plt.xlabel('Normalized Composite Tilt Indicator')
plt.ylabel('Frequency')
# plt.title('Distribution of Normalized Composite Tilt Indicator')

# plt.grid(True)
plt.show()

"""Next order of business is to hypothesis test the assigned labels to make sure the ones labeled tilt are actually tilted to a significant degree.

Once this is done, we can proceed to supervised learning using the labeled data.
"""
...
"""We now transition into using deep learning to predict tilt labels on unseen sequences. Since tilt occurs in only 5% of sequences, the data is imbalanced. We’ll use class weights to help the model learn effectively. Later, you can also explore other methods (such as oversampling or focal loss) to address imbalance."""
...
df = pd.read_csv("/content/drive/MyDrive/Colab Notebooks/Senior Thesis/Code/Redone/tilt_metrics_labeled.csv")
...
"""The last step is to clean a dataset from another platform such as ABS, extract features, and predict labels for new data. Run analysis again for unseen data."""